...
Vagrant.configure("2") do |config|
  ...
  config.vm.hostname = "ocp.vagrant.local"
  config.vm.network "private_network", ip: "192.168.200.5", virtualbox__intnet: "INTERNAL NET"

  config.vm.provision "SUBSCRIBE", upload_path: "/home/vagrant/vagrant-shell", type: "shell", path: './scripts/subscribe.sh', args: "#{secrets['USERNAME']} #{secrets['PASSWORD']} #{secrets['POOL']} #{conf['OCP_RELEASE']} #{conf['ANSIBLE_VERSION']}"
  config.vm.provision "file", source: "./hosts_#{configuration['OCP_RELEASE']}.yml", destination: "/tmp/ocp_inventory.yml"
  config.vm.provision "NGINX", upload_path: "/home/vagrant/vagrant-shell", type: "shell", path: './scripts/openshift.sh', args: "#{conf['OCP_RELEASE']} #{conf['OCP_VERSION']}"
